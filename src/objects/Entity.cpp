...
Entity::~Entity()
{
    delete entitySprite_, entityRect_,errorTexture_;
}

void Entity::initTextures(std::vector<std::string> paths)
{
...
{
    target.draw(*entityRect_);
    target.draw(*entitySprite_);
}
...
void Entity::checkRectCollision(std::vector<sf::RectangleShape*> rects)
{
...
void Entity::checkGroundCollision(sf::RectangleShape& groundRect)
{
...
void Entity::applyFriction(float& walkSpeed, float friction)
{
    if (walkSpeed > 0.f)
    {
