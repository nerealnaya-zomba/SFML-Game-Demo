...
const std::unordered_map<std::string, std::pair<std::string, std::string>>& russianItemText()
{
    static const std::unordered_map<std::string, std::pair<std::string, std::string>> text = {
        {"Item_01.png", {"Синяя фляга", "Холодное синее зелье расширяет внутренний запас. Больше энергии - больше выстрелов до передышки."}},
        {"Item_03.png", {"Красная фляга", "Густое красное зелье для охотников, которые готовы принять удар и идти дальше."}},
        {"Item_04.png", {"Зеленая фляга", "Зеленая смесь одновременно укрепляет тело и запас энергии."}},
        {"Item_06.png", {"Живая веточка", "Гибкая веточка помнит рост вверх. Прыжок становится чище и выше."}},
        {"Item_08.png", {"Шляпка мухомора", "Ядовитый маленький талисман. Опасная магия заставляет выстрелы кусаться сильнее."}},
        {"Item_12.png", {"Книга искр", "Боевой учебник с резкими схемами стрельбы. Заряды срываются с руки быстрее."}},
        {"Item_13.png", {"Кодекс дальнего пути", "Эта книга отмечает, куда должен долететь заряд, прежде чем тьма его проглотит."}},
        {"Item_14.png", {"Шипастый гримуар", "Жестокие заметки на полях учат каждый снаряд попадать с большей силой."}},
        {"Item_15.png", {"Глава шквала", "Страницы сами перелистываются, заставляя руки стрелять и двигаться без паузы."}},
        {"Item_05.png", {"Амулет стража", "Тяжелый оберег от плохих концов. Он укрепляет тело и быстрее возвращает рывок."}},
        {"Item_11.png", {"Книга облачного шага", "Бледный урок о том, как приземляться позже, чем ждет мир. Дает один прыжок в воздухе."}},
        {"Item_20.png", {"Книга разломного рывка", "Чернила складываются в короткие темные коридоры. Рывок сильнее и быстрее восстанавливается."}},
        {"Item_17.png", {"Книга мягкого падения", "Тихое заклинание превращает падение в выбор, а не в ошибку."}},
        {"Item_18.png", {"Кодекс спешки", "Беспокойная книга для быстрых ног. Увеличивает разгон и максимальную скорость."}},
        {"Item_21.png", {"Ключ врат", "Ритуальный ключ откликается на все пути. Он усиливает здоровье, энергию и боевую концентрацию."}},
        {"Item_07.png", {"Кукла вуду", "Злая кукла расщепляет боль на проклятый веер игл."}},
        {"Item_19.png", {"Пламенная книга", "Книга заклинаний катит медленные огненные сферы сквозь скопления врагов."}},
        {"Item_22.png", {"Ночной глаз", "Бдительный глаз раскрывается прямым лучом и прошивает целые линии врагов."}},
        {"Item_29.png", {"Могильная корона", "Королевский ритуал приговора. Один холодный выстрел проходит сквозь броню и кости."}},
        {"Item_33.png", {"Штормовой колпак", "Кривой колпак мага выпускает несколько быстрых зарядов за один вдох."}}
    };
    return text;
}
...
std::string getWeaponKindLabel(Item::WeaponKind kind)
{
    switch (kind)
    {
        case Item::WeaponKind::AshenBolt:
            return Localization::isRussian() ? "Обычный заряд" : "Basic bolt";
        case Item::WeaponKind::Gravepiercer:
            return Localization::isRussian() ? "Пробивающий выстрел" : "Piercing shot";
        case Item::WeaponKind::PyreOrb:
            return Localization::isRussian() ? "Взрывная сфера" : "Explosive orb";
        case Item::WeaponKind::StormNeedler:
            return Localization::isRussian() ? "Серия зарядов" : "Multi-shot";
        case Item::WeaponKind::DreadPrism:
            return Localization::isRussian() ? "Проклятый веер" : "Cursed fan";
        case Item::WeaponKind::NightfallBeam:
            return Localization::isRussian() ? "Пробивающий луч" : "Piercing beam";
    }
...
std::string buildStatsText(const Item& item)
{
    ...
    {
        stream << (Localization::isRussian() ? "Энергия" : "Energy") << " +" << stats.maxEnergy << '\n';
    }
    if(stats.damage != 0)
    {
...
void Shop::initializeItems()
{
    ...
    for (auto &&itemData : loadedData)
    {
        ...
        Item::Stats stats =
        {
            ...
            itemData["Stats"].value("MaxEnergy", 0),
